...
public class HDFSUtils {
    ...
    private HDFSUtils() {
        ...
        config.addResource("dev/core-site.xml");
        config.addResource("dev/hdfs-site.xml");
    ...
    public Boolean copyFromFile(InputStream src, String dst) {
        return handleProcess((fs) -> {
            byte[] buf = new byte[1024];//缓存区
            FSDataOutputStream f = fs.create(new Path(dst), true);
            while((src.read(buf)) != -1) { //读取长度不为-1，继续读取
                f.write(buf); //读取内容之后马上写入目标数据源
            }
            f.flush();//输出
            f.close();
            return true;
    ...
    private <T> T handleProcess(Processor<T> processor) {
        try {
            ...
            System.err.println("访问 HDFS 的时候出现异常" + e.toString());
