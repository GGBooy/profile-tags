package cn.itcast.tags.platform.service.impl;
...
import cn.itcast.tags.platform.entity.po.TagPo;
import cn.itcast.tags.platform.repo.ModelRepo;
import cn.itcast.tags.platform.repo.TagRepo;
import cn.itcast.tags.platform.service.Engine;
...
public class TagAndModelServiceImpl implements TagAndModelService {
    ...
    public void updateModelState(Long id, Integer state) {
        ...
        if (state == ModelPo.STATE_ENABLE) {
            //启动流程
            String jobId = engine.startModel(convert(modelPo));
            System.out.println("start job: " + jobId);
            modelPo.setName(jobId); // 为了关闭
        }
