...
/**
 * 自定义外部数据源：从HBase表加载数据和保存数据值HBase表的Relation实现
 */
case class HBaseRelation(context: SQLContext,
                         params: Map[String, String],
                         userSchema: StructType) extends BaseRelation
...
// 2. 设置读HBase表的名称
    ...
    // TODO: 3.3. 设置过滤条件，格式为：modified[GE]20190601,modified[LE]20191201
    val filterConditions: String = params.getOrElse(
      HBASE_TABLE_FILTER_CONDITIONS, null
    )
    if(null != filterConditions && filterConditions.length > 0){
      // 3.3.1. 构建FilterList对象
      val filterList: FilterList = new FilterList()
      // 3.3.2. 依据条件语句创建Filter对象
      filterConditions.split(SPERATOR).foreach{filterCondition =>
        // i. 解析FILTER CLAUSE
        val condition: Condition =
          Condition.parseCondition(filterCondition)
        // ii. 创建SingleColumnValueFilter对象
        val filter: SingleColumnValueFilter = new
...
override def insert(data: DataFrame, overwrite: Boolean): Unit = {
  ...
  putsRDD.saveAsNewAPIHadoopFile(
    s"/apps/hbase/${params(HBASE_TABLE)}-" + System.currentTimeMillis(),
    classOf[ImmutableBytesWritable], //
    classOf[Put], //
